    ...
    update: function (opts, when) {
      ...
      Object.keys(opts).forEach(function (k) {
        ...
        if (k == 'midiNote' || k == 'freq') {
          ...
          audioNodes.leftfilter.frequency.linearRampToValueAtTime(freq + (freq / (2 + Math.random())), Math.random())
          audioNodes.rightfilter.frequency.linearRampToValueAtTime(freq + (freq / (2 + Math.random())), Math.random())
          audioNodes.mainfilter.frequency.linearRampToValueAtTime(freq + (freq / (1.5 + Math.random())), Math.random())

          audioNodes.osc1.frequency.setValueAtTime(freq / 4.0, 0)
          audioNodes.osc2.frequency.setValueAtTime(freq / 4.0, 0)
          audioNodes.oscsine.frequency.setValueAtTime(freq / 4.0, 0)
          // add some funk to that
          audioNodes.osc1.detune.setValueAtTime(audioNodes.settings.detune * -Math.random(), 0)
          audioNodes.osc2.detune.setValueAtTime(audioNodes.settings.detune * Math.random(), 0)
        } else {
          // just an ADSR value
